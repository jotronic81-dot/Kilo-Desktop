...
import { expectKiloVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveKilo?: typeof resolveWslKilo
...
export function createWslServersController(
  ...
  const setKiloCheck = (distro: string, check: WslKiloCheck) => {
    setState({
      kiloChecks: {
        ...state.kiloChecks,
        [distro]: check,
  ...
  const checkKilo = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveKilo ?? resolveWslKilo)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return kiloCheck(distro, resolved, version, appVersion)
  }

  const refreshKiloCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setKiloCheck(distro, await checkKilo(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const kiloChecks = await Promise.all(
      unique
        ...
        .filter((distro) => !state.kiloChecks[distro])
        .map(async (distro) => [distro, await checkKilo(distro, opts)] as const),
    )
    if (kiloChecks.length) {
      setState({ kiloChecks: { ...state.kiloChecks, ...Object.fromEntries(kiloChecks) } })
    }
  }
  ...
  const refreshKiloCheckBackground = (id: string, distro: string) => {
    void checkKilo(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setKiloCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl kilo check failed", { id, distro, message })
      })
  }

  const refreshKiloChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkKilo(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setKiloCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl kilo check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshKiloCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installKilo(name: string) {
      await runJob({ kind: "install-kilo", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslKilo(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "Kilo installation failed")
        }
        await refreshKiloCheck(name, { signal: abort.signal })
        expectKiloVersion(state.kiloChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.kiloChecks, distro) : {}),
      })
    },
...
function kiloCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "kilo is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "kilo is installed but could not run",
    }
  }
  return {
    distro,
