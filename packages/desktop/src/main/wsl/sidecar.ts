...
import { type WslCommandLine, resolveWslKilo, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const kilo = await resolveWslKilo(distro)
  if (!kilo) throw new Error(`Kilo is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export KILO_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export KILO_CLIENT=desktop",
    `export KILO_SERVER_USERNAME=${shellEscape(username)}`,
    `export KILO_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(kilo)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
