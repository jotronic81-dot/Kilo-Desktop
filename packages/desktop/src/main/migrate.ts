...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.kilo.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `kilo.settings.dat` is special: it maps to the `kilo.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"kilo.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // kilo.settings.dat → the electron settings store ("kilo.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "kilo.global.dat").
  const storeName = filename === "kilo.settings.dat" ? "kilo.settings" : filename
