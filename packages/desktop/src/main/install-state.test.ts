...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Kilo launch", () => {
    expect(hasExistingAppState([file("kilo.settings")])).toBe(true)
    expect(hasExistingAppState([file("kilo.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("kilo")])).toBe(true)
  })
})
