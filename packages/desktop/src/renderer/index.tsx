...
import type { UpdaterState } from "@kilocode-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@kilocode-ai/ui/logo"
import { useTheme } from "@kilocode-ai/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.KILO_CHANNEL === "prod" &&
...
const deepLinkEvent = "kilo:deep-link"
...
function windowLastActiveUrlKey(windowID: string) {
  return `kilo.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("kilo.global.dat").getItem("language")
